...
public class ReceptionController implements Initializable
{
    ...
    @Inject IRepairReception receptionService;
    @Inject IRepairDiagnostic diagnosticService;
    @Inject IManageResource resourceService;

    Reception reception = new Reception();
    Diagnostic diagnostic = new Diagnostic();
    ObservableList<Notice> newEtats;
    ObservableList<Notice> list_etat;
    AtomicInteger rowNbr;
    ObservableList<Default> list_defaut;
    ...
    @FXML
    private ComboBox<Default.Groupe> libelle_defaut;
    @FXML
    private TextArea description_defaut;
    @FXML
    private TableView<Default> table_defaut;
    @FXML
    private TableColumn<Default, String> defaut_libelle;
    @FXML
    private TableColumn<Default, String> defaut_description;
    @FXML
    ...
    @FXML
    private TableColumn<Default, String> solution_libelle;
    @FXML
    private TableColumn<Default, String> solution_description;
    ...
    void add_diagnostic() {
        if (list_defaut.isEmpty()) {
            ...
            diagnostic.setDescription(solution_diagnostic.getText());
            ...
            diagnostic.setDefaults(list_defaut);
            diagnostic.setSolutions(list_solution);

            try {
                diagnostic = diagnosticService.createDiagnostic(
                        new RepairDiagnosticRequest(
                                diagnostic.getDescription(),
                                diagnostic.getCompteur(),
                                diagnostic.getDateCreation(),
                                diagnostic.getDefaults(),
                                diagnostic.getSolutions(),
                                diagnostic.getProfile(),
                                diagnostic.getDossier()
                        )
                );
    ...
    @FXML
    void add_defaut() {
        Default defaut = new Default(Default.Groupe.MOTOPROPULSEUR.name(), Default.Groupe.MOTOPROPULSEUR, description_defaut.getText().trim());
        diagnostic.addDefault(defaut);
        table_defaut.setItems(FXCollections.observableArrayList(diagnostic.getDefaults()));
        description_defaut.setText("");
    }
    
    @FXML
    void remove_defaut() {
        Default defaut = table_defaut.getSelectionModel().getSelectedItem();
    ...
    void save_reception(ActionEvent event) {
        ...
        try {
            reception = receptionService.createReception(
                    new RepairReceptionRequest(
                            reception.getCompteur(),
                            reception.getCommentaire(),
                            reception.getObservation(),
                            reception.getDateCreation(),
                            reception.getNotices(),
                            reception.getProfile(),
                            reception.getDossier()
                    )
            );
            AlertInfo.getInstance().setTitle("Info");
    ...
    void readAll() {
        Reception reception = receptionService.readReception(receptionID);
    ...
    public void getDossiers(){
        this.allDossier = dossierService.listDossier();
    ...
    public void getProfiles(){
        this.allProfile = resourceService.listProfile();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        libelle_defaut.setItems(FXCollections.observableArrayList(Default.Groupe.values()));
        ...
        profile_diagnostic.setConverter(new StringConverter<Profile>() {
            ...
            public Profile fromString(String string) {
                ...
                return resourceService.findProfile(Long.parseLong(id));
            }
        });
        ...
        dossierAutomobile.setConverter(new StringConverter<Dossier>() {
            ...
            public Dossier fromString(String string) {
                ...
                String id = data[0].trim();
                return dossierService.findDossier(Long.parseLong(id));
            }
        });
